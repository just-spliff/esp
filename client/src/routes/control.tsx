...
// ===================== CONFIG =====================
const BROKER_URL = "wss://caddy-production-fba2.up.railway.app/mqtt";
...
function estimateMagnetPct(state: EspState, nowMs: number) {
  if (typeof state.magOutPct === "number") return clamp(state.magOutPct, 0, 100);
...
const StatusDot = ({ active, label }: { active: boolean; label: string }) => (
  <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground bg-secondary/50 px-2 py-1 rounded-full border border-border/50">
    <div className={`h-2 w-2 rounded-full transition-colors duration-300 ${active ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]" : "bg-red-500/50"}`} />
    <span>{label}</span>
  </div>
);
          ...
          <CardContent className="space-y-4 pt-4">
            <div className="space-y-3">
              <div className="space-y-1">
                <Label className="text-xs uppercase text-muted-foreground tracking-wider">Użytkownik</Label>
                <Input value={username} onChange={(e) => setUsername(e.target.value)} className="bg-slate-50" />
              </div>
              <div className="space-y-1">
                <Label className="text-xs uppercase text-muted-foreground tracking-wider">Hasło</Label>
      ...
      <div className="mx-auto max-w-[1600px] space-y-6">
        ...
        <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between rounded-xl bg-white p-4 shadow-sm border border-slate-100">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-lg bg-slate-900 flex items-center justify-center text-white font-bold">FS</div>
            <div>
              <h1 className="text-lg font-bold leading-tight tracking-tight">Panel Sterowania</h1>
              <div className="text-xs font-mono text-muted-foreground flex items-center gap-2">ID: {DEVICE_ID}</div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Card className="border-l-4 shadow-sm overflow-hidden relative" style={{ borderLeftColor: COLOR_MAGNET }}>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="text-3xl font-bold tabular-nums tracking-tighter text-slate-900">
                    {Math.round(currentMag)} <span className="text-base font-normal text-muted-foreground">%</span>
                  </div>
                </CardContent>
              </Card>
            </div>
            ...
            <Card className="shadow-sm border-slate-200">
              <CardHeader className="pb-3 border-b border-slate-100 bg-slate-50/50">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base font-semibold">Konfiguracja</CardTitle>
                  <div className="w-[140px]">
                    <Select value={uiMode} onValueChange={(v) => setUiMode(v as any)} disabled={!connected}>
                      <SelectTrigger className="h-8 bg-white text-xs font-medium"><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="space-y-4">
                      <div className="flex items-end justify-between">
                        <Label className="text-xs uppercase text-muted-foreground">Moc wyjściowa</Label>
                        <div className="text-2xl font-mono font-bold text-violet-600">{manualPct}%</div>
                      </div>
                      <Slider
                        className="py-2 cursor-pointer"
                        value={[manualPct]} min={0} max={100} step={1}
                        disabled={!connected}
                        onPointerDownCapture={() => { draggingRef.current = true; }}
                        onPointerUpCapture={() => { draggingRef.current = false; }}
                        onPointerCancelCapture={() => { draggingRef.current = false; }}
                        onValueChange={(v) => { setManualPct(v[0] ?? 0); }}
                        onValueCommit={(v) => {
                          const pct = v[0] ?? 0;
                ...
                {uiMode === "pulse" && (
                  <div className="space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="space-y-1.5">
                      <Label className="text-xs text-muted-foreground">Rodzaj fali</Label>
                      <Select value={pulseWave} onValueChange={(v) => setPulseWave(v as any)} disabled={!connected}>
                        <SelectTrigger className="bg-slate-50"><SelectValue /></SelectTrigger>
                        <SelectContent><SelectItem value="sine">Sinusoida (SINE)</SelectItem><SelectItem value="square">Prostokąt (SQUARE)</SelectItem></SelectContent>
                      </Select>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
          ...
          <Card className="h-full min-h-[600px] flex flex-col shadow-sm border-slate-200">
            <CardHeader className="flex flex-row items-center justify-between pb-2 border-b border-slate-50">
              ...
              <div className="flex items-center gap-4 text-xs font-medium">
                ...
                <div className="flex items-center gap-1.5 text-violet-600"><div className="h-2 w-4 rounded-full bg-violet-500" /> Magnes</div>
              </div>
            </CardHeader>
                  ...
                  <LineChart data={viewData} margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip
                      ...
                      formatter={(value, name) => {
                        if (name === "pressure") return [<span style={{ color: COLOR_PRESS }}>{Number(value).toFixed(1)} g</span>, "Nacisk"];
                        if (name === "magnet") return [<span style={{ color: COLOR_MAGNET }}>{Math.round(Number(value))}%</span>, "Magnes"];
                        return [value, name];
                      }}
                    />
                    ...
                    <Line
                      ...
                      dot={false} stroke={COLOR_MAGNET} strokeWidth={2} strokeOpacity={0.7}
                      isAnimationActive={false}
                    />
                  </LineChart>
                </ResponsiveContainer>
